...
const EditTask = ({ show, toggle, todoObj, updateTask }) => {
  ...
  useEffect(() => {
    setTaskname(todoObj.task_name);
    setTaskdescription(todoObj.task_description);
    setStatus(todoObj.status);
    setPriority(todoObj.priority);
    setEndDateTime(todoObj.endtime);
  }, []);
  ...
  const handleSubmit = () => {
    ...
    console.log(tasklistobj.id);
    updateTask(tasklistobj);
